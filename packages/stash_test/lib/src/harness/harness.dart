...
typedef TestContextBuilder<S extends Stat, E extends Entry<S>,
        T extends Store<S, E>>
    = TestContext<S, E, T> Function(ValueGenerator generator);

/// [VaultTestContext] builder function
typedef VaultTestContextBuilder<T extends Store<VaultStat, VaultEntry>>
    = VaultTestContext<T> Function(ValueGenerator generator);

/// [CacheTestContext] builder function
typedef CacheTestContextBuilder<T extends Store<CacheStat, CacheEntry>>
    = CacheTestContext<T> Function(ValueGenerator generator);

/// Store builder function
typedef StoreBuilder<S extends Stat, E extends Entry<S>, T extends Store<S, E>>
    = Future<T> Function();

/// Cache builder function
typedef CacheBuilder<T extends Store<CacheStat, CacheEntry>>
...
void check<S extends Stat, E extends Entry<S>, T extends Store<S, E>>(
    TestContext<S, E, T> ctx, dynamic actual, dynamic matcher, String reason) {
...
Vault<V> newDefaultVault<V, T extends Store<VaultStat, VaultEntry>>(
  T store, {
  String? name,
  Clock? clock,
}) {
...
Cache<V> newDefaultCache<V, T extends Store<CacheStat, CacheEntry>>(T store,
    {String? name,
    ExpiryPolicy? expiryPolicy,
    KeySampler? sampler,
    EvictionPolicy? evictionPolicy,
...
abstract class EntryBuilder<S extends Stat, E extends Entry<S>> {
  /// A value generator
...
abstract class TestContext<S extends Stat, E extends Entry<S>,
    T extends Store<S, E>> implements EntryBuilder<S, E> {
  @override
...
abstract class VaultTestContext<T extends Store<VaultStat, VaultEntry>>
    extends TestContext<VaultStat, VaultEntry, T> {
  ...
  /// * [store]: The [Store]
  /// * [name]: The name of the vault
  Vault<V> newVault<V>(T store, {String? name}) {
    return newDefaultVault<V, T>(store, name: name);
  }
...
abstract class CacheTestContext<T extends Store<CacheStat, CacheEntry>>
    extends TestContext<CacheStat, CacheEntry, T> {
